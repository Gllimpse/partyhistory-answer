...
<script type="text/javascript">
    ...
    window.onload = function () {
        ...
        <%--let len =<%=ranksLen%>--%>
        var trow = getDataRow(1, <%=ranks.get(0).account%>, <%=ranks.get(0).score%>);
        tbody.appendChild(trow)
        trow = getDataRow(2, <%=ranks.get(1).account%>, <%=ranks.get(1).score%>);
        tbody.appendChild(trow)
        trow = getDataRow(3, <%=ranks.get(2).account%>, <%=ranks.get(2).score%>);
        tbody.appendChild(trow)
        trow = getDataRow(4, <%=ranks.get(3).account%>, <%=ranks.get(3).score%>);
        tbody.appendChild(trow)
        trow = getDataRow(5, <%=ranks.get(4).account%>, <%=ranks.get(4).score%>);
        tbody.appendChild(trow)
    }
